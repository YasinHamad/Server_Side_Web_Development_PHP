...
<?php
  ...
  if(isset($_GET['add'])){
    add_course_to_user_list($_SESSION['user_name'], $_GET['course_id']);
  }
  if(isset($_GET['remove'])){
    remove_course_from_user_list($_SESSION['user_name'], $_GET['course_id']);
  }

    $teacher_id = get_teacher_id_from_database($_GET['course_id']);
    $teacher_name = get_teacher_name_from_database($teacher_id);
    $teacher_email = get_teacher_email_from_database($teacher_id);

    $course_id[] = $_GET['course_id'];

    $course_info = get_all_info_about_these_courses($course_id);
...
<header>
  ...
  <div class="course_info" style="margin: 100px 0;padding: 0 20px;">
    <div class="container">
      <div class="course-title"><b>Course titel : </b><?php echo $course_info[0]['course_title'] ;?></div><br>
      <div class="course-presenter"><b>Presented by : </b><a href="teacher_view.php?teacher_id=<?php echo $teacher_id; ?>&teacher_name=<?php echo $teacher_name; ?>&teacher_email=<?php echo $teacher_email; ?>"><?php echo $teacher_name; ?></a></div><br>
      ...
      <a href="course_view.php?add=add&course_id=<?php echo $course_info[0]['course_id'] ;?>" style="background-color: black;color: white;padding: 5px 10px;text-decoration: none;<?php if(isset($_SESSION['user_name'])){ echo "display: initial;"; }else {echo "display: none;";} ?>">enroll</a><br><br>
      <a href="course_view.php?remove=remove&course_id=<?php echo $course_info[0]['course_id'] ;?>" style="background-color: black;color: white;padding: 5px 10px;text-decoration: none;<?php
        if(isset($_SESSION['user_name'])){ 
          if(does_user_has_this_course_in_his_list($_SESSION['user_name'], $course_info[0]['course_id'])) echo "display: initial;";
