...
<?php
  if(isset($_POST['submit'])){
    ...
    if(check_full_name($full_name, $correct_full_name)){
      if(check_teacher_user_name($user_name, $correct_user_name)){
        if(check_email($email, $correct_email)){
          if(check_password($password, $correct_password)){
            $_SESSION['teacher_user_name'] = $user_name;
            $_SESSION['teacher_name'] = get_first_name($full_name);
            $_SESSION['teacher_full_name'] = $full_name;
            $_SESSION['teacher_email'] = $email;
            add_teacher_to_database($full_name, $user_name, $password, $email);
            // print_r($_SESSION);
  ...
  <main>
    <form action="teacher_sign_up.php" method="post">
      ...
      <div class="input-container">
        ...
        <input type="text" id="full_name" placeholder="full name" required name="full_name" value="<?php if(isset($_POST['submit'])){echo $full_name;} ?>">
        <?php 
          if(isset($_POST['submit'])){
      ...
      <div class="input-container">
        ...
        <input type="email" id="email" placeholder="example@gmail.com" required name="email" value="<?php if(isset($_POST['submit'])){echo $email;}?>">
        <?php 
          if(isset($_POST['submit'])){
